...
use Wegar\User\model\enum\UserIdentifierTypeEnum as UID;
use Wegar\User\model\enum\UserLogLevelEnum as ULLevel;
use Wegar\User\model\UserModel as User;
use Wegar\User\module\UserModule as UM;
...
class BasicController
{
  ...
  function login(string $identifier, string $password = '', string $code = '', string $sign = '', bool $create = false): Response
  {
    ...
    if ($password) {
      ...
      UM::log('system', ULLevel::INFO, '用户登录', [
      ...
      if (!isset($userinfo->phone) && !isset($userinfo->email)) {
        if ($create) {
          $user = UM::createUser(UID::PHONE, $identifier);
          UM::log('system', ULLevel::INFO, '用户创建', [
            'ip' => request()->getRealIp(false),
          ], $user->id);
        } else {
          throw new BusinessException('用户不存在');
    ...
    UM::loginUserById($user->id);
    return json_success(UM::toReadable($user));
  }
  ...
  function info(Request $request): Response
  {
    ...
    $user = UM::getUserById($current_user->id);
    $current_user = UM::toReadable($user);
  ...
  {
    UM::log('system', ULLevel::INFO, '用户登出', [
      'ip' => request()->getRealIp(false),
    ]);
    return json_success(UM::logoutUser());
  }
